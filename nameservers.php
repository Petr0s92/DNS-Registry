...
?>
                ...
                <div class="mainsubtitle_bg">
                    <div class="mainsubtitle"><a href="javascript: void(0)" id="button2">List all Nameservers</a> | <?if ($_GET['action'] == 'edit'){?> <a href="index.php?section=<?=$SECTION;?>&action=add" class="add"><span>Add New Nameserver</span></a><?}else{?><a href="javascript: void(0)" id="button" class="add"><span>Add New Nameserver</span></a><?}?><?if ($_GET['ns'] && $_GET['domain'] && $_GET['domain_id']){?> | <a href="index.php?section=domain_ns&domain=<?=$_GET['domain'];?>&action=edit&id=<?=$_GET['domain_id'];?>" class="back">Back to Domain edit</a><?}?></div>
                </div> 
